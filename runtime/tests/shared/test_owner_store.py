...
from runtime.shared.db.owner.owner_model import State, Owner
...
def make_owner():
    ...
    owner = OwnerHelper.create_owner(owner_hash="owner_" + "A" * 43,
                                     salt="B" * 22,
...
def test_update_owner_field(ddb_table):
    ...
    owner = make_owner()
    store.put_owner(owner)
    store.update_owner_field(owner.owner_hash, "state", "blocked")
    loaded = store.get_owner(owner.owner_hash)
